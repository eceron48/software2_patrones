...
public class EncryptMessage extends MessageDecorator {
	...
	try {  Key key = new SecretKeySpec(password.getBytes(), "AES");
	
	Cipher c = Cipher.getInstance("AES"); 
	
	c.init(Cipher.ENCRYPT_MODE, key);  byte[] encVal = c.doFinal(message.getContent().getBytes());
	
	String encryptedValue = new BASE64Encoder().encode(encVal);  message.setContent(encryptedValue);
	
	return message;  } catch (Exception e) {
